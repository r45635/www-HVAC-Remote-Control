...
class HVAC_CMD:
	...
	# BroadLink Sepecifc Headr for IR command start with a specific code
	...
	def send_cmd(self, to_host, to_mac, to_devtype="RM2"):
		...
		self.device = broadlink.rm(host=(to_host,80), mac=bytearray.fromhex(to_mac),devtype=to_devtype)
		#self.device = broadlink.rm(host=(to_host,80), mac=bytearray.fromhex(to_mac))
		if (self._log):
		...
		myhex = myhex.replace(' ', '').replace('\n', '')
		myhex = myhex.encode('ascii', 'strict')
		self.device.send_data(binascii.unhexlify(myhex))
		if (self._log):
			print("Code Sent....")
...
parser.add_argument('-F', '--fan', action='store', dest='HVAC_FAN_MODE', default='A',
                    help='Define Fan speed : A=Automatic*, S=Silent, 1..5: Speed')				

parser.add_argument('-host', '--host', action='store', dest='HVAC_HOST', default='192.168.2.96',
                    help='Define RM Host id to contact: -host=192.168.2.96')				
parser.add_argument('-mac', '--mac', action='store', dest='HVAC_MAC', default='34ea348a35ee',
                    help='Define RM Mac Address id to contact: -mac=34ea348a35ee')				
...
if (results.HVAC_FAN_MODE[0:1] == 'S'):
	...
	MyHVAC_cmd.Fan	= MyHVAC_cmd.HVAC_Fan.Speed_1
elif (results.HVAC_FAN_MODE[0:1] == '2'):
	MyHVAC_cmd.Fan	= MyHVAC_cmd.HVAC_Fan.Speed_2
elif (results.HVAC_FAN_MODE[0:1] == '3'):
	MyHVAC_cmd.Fan	= MyHVAC_cmd.HVAC_Fan.Speed_3
elif (results.HVAC_FAN_MODE[0:1] == '4'):
	MyHVAC_cmd.Fan	= MyHVAC_cmd.HVAC_Fan.Speed_4
elif (results.HVAC_FAN_MODE[0:1] == '5'):
	MyHVAC_cmd.Fan	= MyHVAC_cmd.HVAC_Fan.Speed_5
else:
...
try:
	MyHVAC_cmd.send_cmd(to_host=results.HVAC_HOST, to_mac=results.HVAC_MAC)
	print('RUN=OK')
except:	
	print('RUN=NOK')
...
# 
#time.sleep(5)
#MyHVAC_cmd.send_cmd(to_host="192.168.2.100", to_mac="78 0f 77 00 dd 33")
